...
STARTER_ARTICLES: list[dict] = [
    ...
    # ── The Reports ───────────────────────────────────────────────────────────
    {
        ...
        "body": (
            ...
            '\n'
            '- Discovery finds locations within about 50 miles of the flagship. For practices spread across metros, use **Add locations from your own list** — upload the Google Profiles export from SocialClimb as-is (Pulse folds physician profiles into their offices and merges address spellings), or paste names / upload any CSV with a name column; each location is matched to its Google listing and joins the roster.\n'
            ...
            '- **If the website cannot be read by AI assistants** — the report requests the homepage as GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot and Google-Extended, and as a browser. If they are turned away, page 1 says so in red, lists each reader and the answer it got, and explains that a person in a browser (or an assistant fetching one page for them) may still get in while the automated readers that build everyday answers do not. Assistants can still name the organization from Google, Healthgrades and competitors\' pages, but nothing it publishes reaches their answers. The cause is a firewall or bot-management rule, not robots.txt; the fix is an allow rule for the known AI crawlers on public pages, placed first under What to Do First. An amber notice covers robots.txt rules that tell AI crawlers to stay out.\n'
            '- **Physicians Checked** — practice reports look up their physicians in the NPI registry; a physician is linked when a registered location matches a confirmed practice location, and the share linked replaces the model\'s linkage estimate. The table also shows, informationally, whether board certification is stated on the practice\'s own bio pages.\n'
            '- **Confirm the website first.** The form looks up the organization\'s website from its Google listing or your confirmed roster and requires a tick or a correction before the run starts; that one address drives the crawl, the AI-access test and the content analysis.\n'
            '\n'
    ...
    },
    {
        ...
        "body": (
            ...
            '\n'
            '**Confirm the system website.** After discovery, the roster step shows the system\'s website from its Google listing for you to confirm or correct; it is crawled and tested for AI-crawler access. If the site turns AI readers away, page 1 of every file carries the red notice with the readers named and a first step, and the Full Detail file carries the HIGH finding with its remediation plan.\n'
            '\n'
    ...
    # ── Who It's For ──────────────────────────────────────────────────────────
    {
        ...
        "body": (
            ...
            '- **Notes** date what changed (a new website, a review campaign) and appear as markers on the chart and in the Trend Report. Pulse adds two system notes on its own, once per entity: *Method changed* when a hospital\'s snapshot first carries CMS / Leapfrog data verified from the source, and *Method changed (identity)* when a practice\'s snapshot first rests on measured website facts and registry linkage — so a step on that date is not read as a change at the organization.\n'
            '\n'
            '**Needs attention**\n'
            '\n'
    ...
    # ── Getting Started ───────────────────────────────────────────────────────
    {
        ...
        "body": (
            ...
            '- Otherwise pick a report from the Home cards or **Report Creation** in the sidebar, enter the organization or market, confirm the listing, the website and the locations Pulse finds, and run. Rarely-changed settings sit under **Advanced options** on each page.\n'
...
METHODOLOGY_ARTICLES: list[dict] = [
    ...
    },
    {
        ...
        "body": (
            ...
            '- **The Leapfrog Group** — Hospital Safety Grade (A–F), read from hospitalsafetygrade.org\'s city listing (cached per city for a month). Reports distinguish graded, not graded this cycle, not found in that city, and lookup unavailable — and never infer that a hospital declined the Leapfrog Survey, since the Safety Grade is assigned from public data regardless.\n'
            ...
            '- **The organization\'s website, crawled** — schema markup, crawlable physician pages, sitemap, llms.txt and whether AI crawlers are allowed (the homepage is requested as GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot and Google-Extended). For practices this measured result sets the website part of Identity & Machine-Readability. The quality claims the site makes (Leapfrog, CMS, U.S. News, Magnet, Joint Commission) are recorded and checked against the verified values. A bot wall or a robots.txt block is reported on the first page of the report.\n'
            ...
            '- **NPPES** — provider/organization identity and physician rosters; for practices, each sampled physician\'s registered location is matched against the confirmed practice locations to measure physician-to-practice linkage.\n'
    ...
    },
    {
        ...
        "body": (
            ...
            '\n'
            'Where a fact can be measured it is: the CMS star and Leapfrog grade for hospitals, the Google profile facts for every location, and the website\'s machine-readability for practices are set by code from the source, and the model\'s estimate is replaced. Where judgment is needed, the model works from live search and the pages it consulted are listed.\n'
            '\n'
