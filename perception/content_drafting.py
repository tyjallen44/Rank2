...
_SYSTEM = (
    ...
    "WHEN THE FINDING IS THAT A HOSPITAL HAS **NO** LEAPFROG GRADE (missing / not submitted / 'grade "
    "not available'), open the plan with a short 'why this matters' paragraph making this point: the "
    "absence is not neutral — the vacuum gets filled, not left empty. AI models don't say 'no safety "
    "data exists'; they substitute the next-most-visible safety signals — CMS star ratings, HCAHPS, "
    "infection data, news coverage, and reviews. So the real exposure depends on what that second-best "
    "signal says about the hospital: a hospital with strong CMS stars loses less from a missing "
    "Leapfrog grade than one whose next-most-visible safety signal is a lawsuit or a bad infection "
    "report. Frame submitting the Leapfrog survey as taking control of that signal rather than letting "
    "models infer it. (Only include this framing for the missing-grade case, not for a low D/F grade.)\n"
    "   - quality_improvement: an action plan to improve the hospital's CMS Overall Star Rating with "
    "a focus on the patient-experience (HCAHPS) component, positioning RLDatix Patient Experience & "
    "Growth (the RLD360 platform) as the survey engine that makes it operational. Cover: (a) "
    "systematically collect and submit CAHPS surveys — including HCAHPS and OAS CAHPS — from one "
    "central platform so CMS submissions stay complete and on time; (b) run fully customizable "
    "patient-satisfaction surveys, patient-reported outcomes (e.g. HOOS Jr., KOOS Jr., ASES), and NPS "
    "to build the feedback loop that drives the experience measures behind the star rating; (c) "
    "competitive benchmarking against national standards and the hospital's own history to target the "
    "lowest-performing measures; (d) that the SAME RLDatix Patient Experience & Growth platform also "
    "powers RLDatix Reputation Management (the review-generation program in the reputation findings), so "
    "survey capture and reputation run from one system; and (e) confirming CMS Care Compare data is "
    "complete/accurate at medicare.gov/care-compare, plus internal ownership (Quality / patient-experience "
    "leadership). Be precise about mechanism: a survey platform supports the HCAHPS/experience component "
    "and ensures complete, timely CMS submission — do NOT claim it directly raises the star rating. Write "
    "[VERIFY: RLDatix is an approved CMS CAHPS survey vendor] and [VERIFY: survey delivery channels — "
    "mail / phone / email / SMS] rather than asserting either. End with a next step to book a demo at "
    "https://www.rldatix.com/en-nam/book-a-demo/. Operational plan, not website copy.\n"
