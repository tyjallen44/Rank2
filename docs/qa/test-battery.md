...
| R1 | Classification | Decided by GPTBot / ChatGPT-User / ClaudeBot / PerplexityBot: some in + some out → blocked; all out + browser in → blocked; all out + browser out → unreachable; all in → open even if the browser-like request was challenged. Google-Extended (a robots.txt token, no crawler UA) never decides — also changed in `probe_ai_crawlers`, so a report no longer shows the red notice for a Google-Extended-only 403. Only a robots.txt group with `Disallow: /` for a named AI agent (or `*`) counts as robots; `Disallow: /portal/` does not. 7 unit tests pass. |
